...
def build_aggregate_blocks(noun: str, start: str, end: str, who: str, total) -> list:
    ...
    return [
        ...
        {"type": "context", "elements": [{"type": "mrkdwn", "text": f"Data extracted from XDMOD {XDMOD_URL}"}]},
    ]
...
async def on_submit(ack, body, view, client):
    ...
    with DataWarehouse(XDMOD_URL) as dw:
        if fmt == "aggregate":
            ...
            ax.set_title(f"{metric.replace('_',' ').title()} ({start}→{end})")
            ax.set_xlabel("Date")
            ...
            if not is_dm:
                await client.chat_postMessage(
                    channel=target,
                    text=f"<@{body['user']['id']}> here's your chart :chart_with_upwards_trend:",
                )
            await client.files_upload_v2(
                file=buf,
                channels=[target],
                filename=f"{metric}_{start}_{end}.png",
                title=f"{metric} {start}→{end}",
            )
            buf.close()
